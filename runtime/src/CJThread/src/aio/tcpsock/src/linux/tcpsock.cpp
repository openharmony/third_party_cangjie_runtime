...
int TcpsockKeepAliveSet(int fd, const struct SockKeepAliveCfg *cfg)
{
    ...
    if (setsockopt(fd, SOL_TCP, TCP_KEEPIDLE, &cfg->idle, sizeof(unsigned int)) == -1) {
        ret = errno;
        LOG_ERROR(ret, "setsockopt failed, fd: %d, val: %u", fd, cfg->idle);
        return ret;
    }

    if (setsockopt(fd, SOL_TCP, TCP_KEEPINTVL, &cfg->interval, sizeof(unsigned int)) == -1) {
        ret = errno;
        LOG_ERROR(ret, "setsockopt failed, fd: %d, val: %u", fd, cfg->interval);
        return ret;
    }

    if (setsockopt(fd, SOL_TCP, TCP_KEEPCNT, &cfg->count, sizeof(unsigned int)) == -1) {
        ret = errno;
